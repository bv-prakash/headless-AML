...
/**
 * Magento `catalog_default_sort_by` / Luma sorter often uses values outside our small set.
 * Unknown tokens fall back to `position` so GraphQL always receives a valid `sort` object.
 */
...
// ── Filter (mirrors server PLP) ────────────────────────────

const CLIENT_SIDE_FACET_CODES: ReadonlySet<string> = new Set(["product_type"]);

/**
 * @param categoryScopeUid When set (and no URL facet `category_uid`), scopes with `category_uid`
 * instead of numeric `category_id` — some Magento / multi-store setups return no rows for `category_id` only.
 */
...
/**
 * Builds the exact `variables` object sent to Magento for PLP (after stripping client-only facets).
 */
