...
import { isCmsNotFoundGraphqlError } from "./cmsBlocks";
...
/**
 * Plain query string (not `gql`) — Magento's HTTP GraphQL endpoint expects a string
 * body and `magentoGraphqlFetch` sends the `Store` header so the result is scoped
 * to the active store view rather than the default.
 */
const HOME_QUERY = `
...
/**
 * Fetches a CMS page for the active store view.
 *
 * Uses {@link magentoGraphqlFetchSafe} (never throws). Magento PHP fatals / timeouts / 5xx
 * return `null` quietly so Next 16's RSC console-error replay does not surface a
 * "Console MagentoGraphqlError" in the browser dev overlay. CMS-not-found errors
 * (legitimate missing page on a store view) also resolve to `null`.
 */
export async function getHomePage(
  ...
  const fetchByStoreView = async (
    ...
    /**
     * "No such CMS page" is expected when a store view doesn't define the identifier.
     * PHP fatals / timeouts / network errors are already typed and need no dev overlay.
     * Real GraphQL surprises still get a quiet server-only log.
     */
