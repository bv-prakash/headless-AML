...
/** Same shape as {@link CATEGORY_LIST_QUERY} / by-uid query — keep in sync. */
...
/**
 * Category tree for the header nav for the active store view.
 * Uses {@link getCategoryNavRootIdOverride} when set; otherwise `storeConfig.root_category_uid`
 * (from cached {@link getStoreConfig}) + `categoryList` by UID (multi-store Magento); falls back to root id `"2"`.
 * Never throws — returns `[]` so the header can render even if Magento is temporarily unavailable.
 */
...
/**
 * Category-by-url-path is hot during PLP resolution on multi-store / multi-language.
 * Never throws — on a PHP fatal / 5xx the result is `null` (treated like "not found") and
 * the caller can render `notFound()` instead of crashing the whole route.
 */
...
/**
 * URL segments under `/products/[...slug]` vs Magento `url_path` often differ by a leading
 * `products/` segment. Try a few shapes so PLP resolves the same category as Luma.
 */
