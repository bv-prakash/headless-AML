...
/**
 * Safe defaults used when Magento is temporarily unavailable (e.g. PHP
 * OOM in `StoreRepository`). Keeps header / footer / PLP rendering
 * with sensible values instead of surfacing a full error page.
 */
...
/**
 * One Magento `storeConfig` fetch per request for the active store (or
 * per explicit store code). Falls back to {@link FALLBACK_STORE_CONFIG}
 * when Magento returns a PHP fatal so the page still renders.
 */
